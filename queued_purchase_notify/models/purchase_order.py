...
class PurchaseOrder(models.Model):
    ...
    def write(self, values):
        ...
        for sel in self:
            if (sel.partner_ref and sel.partner_ref != "") or (values.get('partner_ref') and values.get('partner_ref') != ""):
    ...
    def button_confirm(self):
        ...
        for sel in self.filtered(
            lambda a: not a.partner_ref or a.partner_ref == ""
        ):
            sel.purchase_notify()
        return return_value
    ...
    def purchase_notify(self):
        ...
        for mail in order_type.purchase_mail_notify_ids:
            self.with_delay(eta=int(mail.time_after_confirmation*3600)).delay_purchase_mail_notify(mail)
        for log in order_type.purchase_log_note_notify_ids:
            self.with_delay(eta=int(log.time_after_confirmation*3600)).delay_purchase_log_notify(log)
        for activity in order_type.purchase_activity_notify_ids:
            self.with_delay(eta=int(activity.time_after_confirmation*3600)).delay_purchase_activity_notify(activity)
    ...
    def delay_purchase_activity_notify(self, activity):
        if (not self.partner_ref or self.partner_ref == "") and self.user_id:
            deadline = fields.Datetime.to_string(datetime.datetime.now() + timedelta(days=activity.time_to_deadline))
